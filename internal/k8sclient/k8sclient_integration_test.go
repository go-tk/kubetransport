...
func TestK8sClientWithIntegration(t *testing.T) {
	te := prepareTestingEnvironment(t)
	t.Run("GetEndpoints", func(t *testing.T) { testK8sClientWithIntegration_GetEndpoints(t, te) })
	t.Run("WatchEndpoints", func(t *testing.T) { testK8sClientWithIntegration_WatchEndpoints(t, te) })
}

func testK8sClientWithIntegration_GetEndpoints(t *testing.T, te *testingEnvironment) {
	kc, err := DoNew(te.Fs, DummyTransportReplacer, te.Env, te.Clock)
	if err != nil {
		t.Fatal(err)
	}

	{
		endpoints, err := kc.GetEndpoints(context.Background(), kc.Namespace(), "abc")
		if err != nil {
			t.Fatal(err)
		}
	...
	{
		endpoints, err := te.Clientset.CoreV1().Endpoints(kc.Namespace()).Create(
			context.Background(),
			&corev1.Endpoints{
		...
		endpoints2, err := kc.GetEndpoints(context.Background(), kc.Namespace(), "abc")
		if err != nil {
			t.Fatal(err)
		}
...

	{
		...
		cancel()
		_, err := kc.GetEndpoints(ctx, kc.Namespace(), "abcd")
...
func testK8sClientWithIntegration_WatchEndpoints(t *testing.T, te *testingEnvironment) {
	kc, err := DoNew(te.Fs, DummyTransportReplacer, te.Env, te.Clock)
	if err != nil {
		t.Fatal(err)
	}

	{
		...
		_ = cancel
		var f bool
		err := kc.WatchEndpoints(ctx, kc.Namespace(), "xyz", "", func(EventType, *Endpoints) bool {
			f = true
			return true
		})
...

	{
		go func() {
			...
			if _, err := te.Clientset.CoreV1().Endpoints(kc.Namespace()).Create(
				context.Background(),
				&corev1.Endpoints{
					ObjectMeta: metav1.ObjectMeta{
						Name: "xyz",
			...
			if _, err := te.Clientset.CoreV1().Endpoints(kc.Namespace()).Update(
				context.Background(),
				&corev1.Endpoints{
					ObjectMeta: metav1.ObjectMeta{
						Name: "xyz",
			...
			if err := te.Clientset.CoreV1().Endpoints(kc.Namespace()).Delete(
				context.Background(),
				"xyz",
		...
		n := 0
		err := kc.WatchEndpoints(context.Background(), kc.Namespace(), "xyz", "", func(eventType EventType, endpoints *Endpoints) bool {
			n++
...

	{
		...
		endpoints, err := kc.GetEndpoints(context.Background(), kc.Namespace(), "ijk")
		if err != nil {
			t.Fatal(err)
		}
		...
		err = kc.WatchEndpoints(context.Background(), kc.Namespace(), "ijk", "", func(eventType EventType, endpoints *Endpoints) bool {
		...
		err = kc.WatchEndpoints(context.Background(), kc.Namespace(), "ijk", endpoints.Metadata.ResourceVersion, func(eventType EventType, endpoints *Endpoints) bool {
